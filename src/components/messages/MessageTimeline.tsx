...
function StageIcon({ Icon, size }: { Icon: any; size?: number }) {
  return (
    <div className="htw-h-9 htw-w-9 htw-flex htw-items-center htw-justify-center htw-rounded-full htw-bg-yellow-500">
      <Icon width={size ?? 14} height={size ?? 14} alt="" color={Color.primaryYellow} />
    </div>
  );
}
...
function ChevronWhite() {
  return (
    <div className="htw-absolute htw--left-3 htw-top-0 htw-h-6">
      <WideChevron direction="e" height="100%" width="auto" color={Color.primaryBluish} />
    </div>
  );
}
...
function ChevronBlue() {
  return (
    <div className="htw-absolute htw--right-3 htw-top-0 htw-h-6">
      <WideChevron direction="e" height="100%" width="auto" color={Color.primaryYellow} />
    </div>
  );
}
...
const styles = {
  ...
  stageBar:
    'htw-w-full htw-h-6 htw-flex htw-items-center htw-justify-center bg-yellow-700 htw-bg-yellow-500 htw-relative',
  ...
  stageIconCircle: 'htw-w-0.5 htw-h-4 htw-bg-yellow-500',
  stageHeader: 'htw-mt-2.5 htw-text-gray-500 htw-text-xs xs:htw-text-sm sm:htw-text-base',
